...
class Suffusion_Category_Posts extends WP_Widget {
	...
	function Suffusion_Category_Posts() {
		$widget_ops = array('classname' => 'widget-suf-cat-posts',
			'description' => __("A widget to pull posts based on various selection criteria. You can use this to display posts from a category, latest posts, random posts, popular posts etc.", "subsuffusion"));

		$control_ops = array('width' => 810);
		$this->WP_Widget("suf-cat-posts", __("subQuery Posts", "suffusion"), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance) {
        ...
        if (trim($all_posts_text)) {
			...
			$ret .= "\t<a class='suf-mag-category-all-posts weekCommentsLink' href='".get_permalink()."'>Comments</a>\n";
